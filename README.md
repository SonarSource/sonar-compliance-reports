...
3. Leave the input empty to release the latest promoted default-branch commit.

`Release Operator` resolves the promoted commit/build itself, reads `compliance-reports/gradle.properties` from that commit,
derives the release version, and creates the required release tag automatically.
...
- If the requested release already has a successful prior release, the operator skips it.
